    ...
    public class MixtureModel
    {
        ...
        private double UpdateExpectation()
        {
            ...
            for (int i = 0; i < AlleleDepths.Count; i++)
            {
                ...
                for (int k = 0; k < Means.Length; k++)
                {
                    ...
                    tempPosts[k] =  MathNet.Numerics.Distributions.Binomial.PMF(Means[k], TotalDepths[i], AlleleDepths[i]) * MixtureWeights[k];
                    ...
                    if (k == (Means.Length - 1) && posteriorsSum[i] == 0)
                    {
                        for (int kk = 0; kk < Means.Length; kk++)
                        {
                            ...
                            tempPosts[kk] = MathNet.Numerics.Distributions.Normal.PDF(Means[kk], Math.Sqrt(TotalDepths[i] * Means[kk] * (1 - Means[kk])),
        ...
        private static double[] CalculatePosteriors(int[] kArray, int[] nArray, double[] means, double[] priors)
        {
            ...
            for (int i = 0; i < means.Length; i++)
            {
                ...
                //MathNet.Numerics.Distributions.Binomial.
                ...
                if (i == (means.Length - 1) && posteriorsSum == 0)
                {
                    ...
                    {
                        tempPosts[ii] = MathNet.Numerics.Distributions.Normal.PDF(means[ii], Math.Sqrt(nArray[i] * means[ii] * (1 - means[ii])),
